...
public class chrome_naverdemo_testcase  {
	...
	public void test_demo(int cnt, int test_cnt, int i, int t_cnt, String user_url) throws Exception  {
			...
			String search_str = tutil.get_parm(test_name, t_cnt, "query");
		    driver.findElement(By.id("query")).sendKeys(search_str);
			...
			tutil.cnt_logging_text(" 조회 내용 : "+tutil.get_parm(test_name, t_cnt, "query"));
			...
			if(wpt_yn.equals("Y")){
				...
				Thread.sleep(1000);
				tutil.Write_csvfile_y(search_str, t_cnt, cnt+1,  i, loadsttm, loadendtm, eventsttm, eventendtm, logstdate, logeddate, user_url);
			}else{
				tutil.Write_csvfile_n(search_str, t_cnt, loadsttm, loadendtm, eventsttm, eventendtm, logstdate, logeddate);
			}
